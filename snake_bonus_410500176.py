...
def sound_image_dis(playsurface):
    picture_path = [".\picture\MAX.png",".\picture\MAX_white.png",".\picture\\none.png",".\picture\\none_white.png"] 
    image = pygame.image.load(".\picture\MAX.png")   #生成圖片 
    image.convert()
    ...
    mouse_pos[1] >= imagerect[1] and mouse_pos[1] <= imagerect[1] + 90:
        ...
        playsurface.blit(pygame.image.load(picture_path[path+1]), imagerect)

    else:
        playsurface.fill(light_grey)
        playsurface.blit(pygame.image.load(picture_path[path]), imagerect)
...
def main():
    ...
    pygame.mixer.music.load(".\music\snake_music.mp3")
    ...
    while start:
        ...
        playsurface.fill(light_grey)
        image = pygame.image.load(".\picture\snaketitle.png")   #生成圖片
        image.convert()
        imagerect = image.get_rect()
